...
class BenchConfig:
    ...
    trace_mode: str = "mixture"
    hot_user_ratio: float = 0.05
    hot_request_ratio: float = 0.7
    session_min_len: int = 1
    session_max_len: int = 8
...
def _generate_request_trace(cfg: BenchConfig) -> List[int]:
    total_requests = cfg.warmup_requests + cfg.measure_requests
    generator = torch.Generator()
    generator.manual_seed(cfg.seed + 999)

    if cfg.trace_mode.lower() == "zipf":
        alpha = 1.2
        ranks = torch.arange(1, cfg.num_users + 1, dtype=torch.float64)
        probs = (1.0 / (ranks ** alpha)).to(dtype=torch.float64)
        probs /= probs.sum()
        requests = torch.multinomial(
            input=probs,
            num_samples=total_requests,
            replacement=True,
            generator=generator,
        ).tolist()

        if cfg.drift_interval:
            shift = cfg.drift_shift if cfg.drift_shift is not None else max(1, cfg.num_users // 4)
            interval = max(1, cfg.drift_interval)
            for start in range(interval, total_requests, interval):
                for idx in range(start, min(start + interval, total_requests)):
                    requests[idx] = (requests[idx] + shift) % cfg.num_users
        return requests

    # Default: mixture of hot/cold with session bursts.
    total_requests = max(total_requests, 1)
    min_len = max(1, cfg.session_min_len)
    max_len = max(min_len, cfg.session_max_len)

    perm = torch.randperm(cfg.num_users, generator=generator)
    hot_count = max(1, int(cfg.num_users * cfg.hot_user_ratio))
    hot_tensor = perm[:hot_count].clone()
    cold_tensor = perm[hot_count:].clone()

    requests: List[int] = []
    next_drift = cfg.drift_interval if cfg.drift_interval else None
    shift_value = cfg.drift_shift if cfg.drift_shift is not None else max(1, cfg.num_users // 4)

    while len(requests) < total_requests:
        session_len = torch.randint(min_len, max_len + 1, (1,), generator=generator).item()
        use_hot = torch.rand(1, generator=generator).item() < cfg.hot_request_ratio or cold_tensor.numel() == 0
        pool = hot_tensor if use_hot or cold_tensor.numel() == 0 else cold_tensor

        if pool.numel() == 0:
            user_id = torch.randint(0, cfg.num_users, (1,), generator=generator).item()
        else:
            idx = torch.randint(0, pool.numel(), (1,), generator=generator).item()
            user_id = pool[idx].item()

        requests.extend([user_id] * session_len)

        if next_drift is not None and len(requests) >= next_drift:
            hot_tensor = torch.remainder(hot_tensor + shift_value, cfg.num_users)
            if cold_tensor.numel() > 0:
                cold_tensor = torch.remainder(cold_tensor + shift_value, cfg.num_users)
            next_drift += cfg.drift_interval
...
def run_benchmark(cfg: BenchConfig, limits: List[BenchmarkLimit]) -> None:
    ...
    print("[kv-bench] Generating request trace...", flush=True)
    final_requests = _generate_request_trace(cfg)
    total_requests = len(final_requests)
    hot_sample = Counter(final_requests[: cfg.measure_requests]).most_common(10)
    print(
        "[kv-bench] Trace ready. Top-10 users in measurement window: "
        + ", ".join(f"{uid}:{cnt}" for uid, cnt in hot_sample),
        flush=True,
    )

    warmup_requests = final_requests[: cfg.warmup_requests]
    measure_requests = final_requests[cfg.warmup_requests : cfg.warmup_requests + cfg.measure_requests]
...
def parse_args():
    ...
    parser.add_argument("--trace-mode", type=str, default=BenchConfig.trace_mode, choices=["mixture", "zipf"], help="Request trace distribution")
    parser.add_argument("--hot-user-ratio", type=float, default=BenchConfig.hot_user_ratio, help="Fraction of users considered hot")
    parser.add_argument("--hot-request-ratio", type=float, default=BenchConfig.hot_request_ratio, help="Probability a session targets hot users")
    parser.add_argument("--session-min-len", type=int, default=BenchConfig.session_min_len, help="Minimum requests per session burst")
    parser.add_argument("--session-max-len", type=int, default=BenchConfig.session_max_len, help="Maximum requests per session burst")
    parser.add_argument("--drift-interval", type=int, default=BenchConfig.drift_interval, help="Requests between hot set drift (optional)")
    parser.add_argument("--drift-shift", type=int, default=BenchConfig.drift_shift, help="ID shift applied on drift (optional)")
