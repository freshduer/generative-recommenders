...
# -*- coding: utf-8 -*-
"""
分析Yelp数据集不同时间窗口下用户交互序列的长度分布和活跃用户数分布
...
def load_yelp_data(data_path: str, show_progress: bool = False) -> pd.DataFrame:
    """加载Yelp评论数据集"""
    logger.info(f"正在加载数据: {data_path}")
    start_time = time.time()
    
    # Yelp数据集格式：JSON Lines，每行一个JSON对象
    # 字段：review_id, user_id, business_id, stars, useful, funny, cool, text, date
    records = []
    chunk_size = 100000  # 每次处理10万条记录
    
    try:
        # 先统计总行数（用于进度条）
        if HAS_TQDM and show_progress:
            total_lines = sum(1 for _ in open(data_path, 'r', encoding='utf-8'))
            logger.info(f"数据文件总行数: {total_lines:,}")
        
        with open(data_path, 'r', encoding='utf-8') as f:
            if HAS_TQDM and show_progress:
                f = tqdm(f, desc="读取数据", unit="行", total=total_lines)
            ...
            for line_num, line in enumerate(f):
                ...
                try:
                    ...
                    if len(records) >= chunk_size:
                        if show_progress:
                            logger.info(f"已处理 {len(records):,} 条记录...")
                
                except json.JSONDecodeError as e:
                    logger.warning(f"第 {line_num+1} 行JSON解析失败: {e}")
                    continue
    ...
    if invalid_count > 0:
        logger.warning(f"发现 {invalid_count} 条无效时间戳记录，将被删除")
    ...
    logger.info(f"唯一商家数: {df['business_id'].nunique():,}")
...
def analyze_sequence_length_by_time_window(
    ...
    """
    # 加载数据
    df = load_yelp_data(data_path, show_progress)
...
def main():
    parser = argparse.ArgumentParser(description='分析Yelp数据集不同时间窗口下用户交互序列长度分布和活跃用户数分布')
    parser.add_argument(
        '--data_path',
        type=str,
        default='~/data/yelp/yelp_academic_dataset_review.json',
        help='数据文件路径（默认: ~/data/yelp/yelp_academic_dataset_review.json）'
    )
    parser.add_argument(
        '--window_type',
        type=str,
    ...
    logger.info("正在加载数据...")
    logger.info("="*80)
    df = load_yelp_data(data_path, args.show_progress)
