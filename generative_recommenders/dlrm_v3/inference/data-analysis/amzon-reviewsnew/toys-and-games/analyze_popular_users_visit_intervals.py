...
def identify_popular_users(
    ...
    # 计算总访问量
    total_visits_all = user_visits_all['visit_count'].sum()
    logger.info(f"Total users in dataset: {total_all_users:,}")
    logger.info(f"Total visits in dataset: {total_visits_all:,}")
    
    # 统计访问次数分布
    single_visit_users = (user_visits_all['visit_count'] == 1).sum()
    multi_visit_users = (user_visits_all['visit_count'] >= 2).sum()
    multi_visit_users_20 = (user_visits_all['visit_count'] >= 20).sum()
    ...
    logger.info(f"  - Users with 20+ visits: {multi_visit_users_20:,} ({multi_visit_users_20/total_all_users*100:.2f}%)")
    ...
    # 计算访问次数 >= min_visits 的用户的总访问量
    total_visits_eligible = user_visits['visit_count'].sum()
    eligible_visits_pct = (total_visits_eligible / total_visits_all * 100) if total_visits_all > 0 else 0
    ...
    logger.info(f"\n访问量统计 (访问次数 >= {min_visits} 的用户):")
    logger.info(f"  - 总访问量: {total_visits_eligible:,} 次")
    logger.info(f"  - 占总访问量的比例: {eligible_visits_pct:.2f}%")
    logger.info(f"  - 平均每个用户访问: {total_visits_eligible/total_eligible_users:.2f} 次" if total_eligible_users > 0 else "  - 平均每个用户访问: 0 次")
    ...
    # 计算选中的热门用户的总访问量
    popular_users_visits = popular_users['visit_count'].sum()
    popular_users_visits_pct = (popular_users_visits / total_visits_all * 100) if total_visits_all > 0 else 0
    ...
    logger.info(f"  Total visits from popular users: {popular_users_visits:,}")
    logger.info(f"  Percentage of total visits: {popular_users_visits_pct:.2f}%")
...
def plot_visit_intervals(
    ...
    gs = fig.add_gridspec(3, 3, hspace=0.3, wspace=0.4)  # Increase wspace for legend
    ...
    # Merge small categories (<5%) as "Others" to avoid label overlap
    ...
    if other_count > 0:
        ...
        plot_counts = pd.concat([main_categories, pd.Series([other_count], index=['Others (<5%)'])])
        plot_colors = list(colors[:len(main_categories)]) + ['lightgray']
    else:
        plot_counts = category_counts
        plot_colors = colors
    
    # Use legend instead of labels on pie chart to avoid overlap
    ...
    # Create legend on the left side to avoid being blocked by other plots
    legend_labels = [f'{label}: {count:,} ({count/total_count*100:.1f}%)' 
                     for label, count in zip(plot_counts.index, plot_counts.values)]
    ax5.legend(
        wedges,
        legend_labels,
        title="Interval Category",
        loc="center right",
        bbox_to_anchor=(-0.15, 0.5),  # Position on the left side
        fontsize=9,
        frameon=True,
        fancybox=True,
        shadow=True
    ...
    fig.savefig(output_path, dpi=200, bbox_inches='tight', pad_inches=0.2)  # Add padding for legend
    plt.close(fig)
...
def main():
    ...
    user_visits_all = df.groupby('user_id', observed=True).size().reset_index(name='visit_count')
    total_visits_all = user_visits_all['visit_count'].sum()
    total_users_all = len(user_visits_all)
    
    # 访问次数 >= min_visits 的用户统计
    eligible_users = user_visits_all[user_visits_all['visit_count'] >= args.min_visits]
    total_visits_eligible = eligible_users['visit_count'].sum()
    eligible_visits_pct = (total_visits_eligible / total_visits_all * 100) if total_visits_all > 0 else 0
    ...
    all_results = {
        'total_statistics': {
            'total_users': int(total_users_all),
            'total_visits': int(total_visits_all)
        },
        'min_visits_threshold': args.min_visits,
        'eligible_users_statistics': {
            'count': int(len(eligible_users)),
            'percentage_of_users': float(len(eligible_users) / total_users_all * 100) if total_users_all > 0 else 0,
            'total_visits': int(total_visits_eligible),
            'percentage_of_total_visits': float(eligible_visits_pct),
            'average_visits_per_user': float(total_visits_eligible / len(eligible_users)) if len(eligible_users) > 0 else 0
        },
        ...
        'popular_users_visit_statistics': {
            'total_visits': int(popular_users_visits),
            'percentage_of_total_visits': float(popular_users_visits_pct)
        },
