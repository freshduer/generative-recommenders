...
# -*- coding: utf-8 -*-
"""
分析淘宝行为数据集不同时间窗口下用户交互序列的长度分布和活跃用户数分布
...
def load_taobao_data(data_path: str, show_progress: bool = False) -> pd.DataFrame:
    """加载淘宝行为数据集"""
    logger.info(f"正在加载数据: {data_path}")
    start_time = time.time()
    
    # 淘宝数据集格式：user_id,item_id,category_id,behavior_type,timestamp
    # 使用chunksize分块读取大文件
    chunks = []
    ...
    try:
        # 先读取一小部分来确定列名
        sample = pd.read_csv(data_path, nrows=5, header=None)
        ...
        if num_cols == 5:
            ...
            # 如果没有5列，尝试自动推断
            column_names = None
            logger.warning(f"检测到{num_cols}列，尝试自动推断列名")
        ...
        if HAS_TQDM and show_progress:
            chunk_iterator = pd.read_csv(
                ...
                dtype={'user_id': 'int64', 'item_id': 'int64', 'timestamp': 'int64'}
            )
...
def analyze_sequence_length_by_time_window(
    ...
    """
    # 加载数据
    df = load_taobao_data(data_path, show_progress)
...
def main():
    parser = argparse.ArgumentParser(description='分析淘宝行为数据集不同时间窗口下用户交互序列长度分布和活跃用户数分布')
    parser.add_argument(
        '--data_path',
        type=str,
        default='~/data/taobao/UserBehavior.csv',
        help='数据文件路径（默认: ~/data/taobao/UserBehavior.csv）'
    )
    parser.add_argument(
        '--window_type',
        type=str,
    ...
    logger.info("正在加载数据...")
    logger.info("="*80)
    df = load_taobao_data(data_path, args.show_progress)
