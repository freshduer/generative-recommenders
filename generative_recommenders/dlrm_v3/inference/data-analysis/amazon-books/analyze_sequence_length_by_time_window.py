...
def load_amazon_data(data_path: str) -> pd.DataFrame:
    """加载Amazon数据集"""
...
def parse_sequences_to_expanded(df: pd.DataFrame, show_progress: bool = False) -> pd.DataFrame:
    """解析序列数据，展开为单条交互记录"""
    logger.info("正在解析序列数据...")
    start_time = time.time()
    ...
    if HAS_TQDM and show_progress:
        iterator = tqdm(df.itertuples(index=False), total=total_rows, desc="解析序列", unit="行")
    else:
        iterator = df.itertuples(index=False)
        if show_progress:
            logger.info(f"总共需要处理 {total_rows:,} 行数据...")
        last_progress = -1
    ...
    for idx, row in enumerate(iterator):
        if not HAS_TQDM and show_progress:
            ...
            if progress != last_progress and progress % 10 == 0:
                logger.info(f"进度: {progress}% ({idx + 1:,}/{total_rows:,})")
        ...
        # 扩展为单条记录
        records.extend(zip([user_id] * len(item_ids), item_ids, timestamps))
    
    logger.info("正在创建DataFrame...")
    expanded_df = pd.DataFrame(records, columns=['user_id', 'item_id', 'timestamp'])
    
    # 转换时间戳为datetime
    expanded_df['datetime'] = pd.to_datetime(expanded_df['timestamp'], unit='s')
    
    elapsed = time.time() - start_time
    logger.info(f"展开后共 {len(expanded_df):,} 条交互记录，耗时 {elapsed:.2f} 秒")
    return expanded_df
...
def plot_active_users_over_time(
    ...
    ax2.axvline(active_users_stats["active_user_count"].mean(), color='red', linestyle='--', linewidth=2, label=f'Mean: {active_users_stats["active_user_count"].mean():.0f}')
    ax2.axvline(active_users_stats["active_user_count"].median(), color='green', linestyle='--', linewidth=2, label=f'Median: {active_users_stats["active_user_count"].median():.0f}')
...
def analyze_sequence_length_by_time_window(
    ...
    # 展开序列数据
    expanded_df = parse_sequences_to_expanded(df, show_progress)
    logger.info(f"时间范围: {expanded_df['datetime'].min()} 到 {expanded_df['datetime'].max()}")
    
    # 计算每个时间窗口内每个用户的序列长度
    user_seq_lengths = compute_user_sequence_lengths_by_window(
        expanded_df, window_type, show_progress
    )
    ...
    # 计算活跃用户数
    active_users = compute_active_users_by_window(expanded_df, window_type, show_progress)
...
def main():
    parser = argparse.ArgumentParser(description='分析不同时间窗口下用户交互序列长度分布和活跃用户数分布')
    parser.add_argument(
        '--data_path',
        type=str,
    ...
    for window_type in window_types:
        ...
        # 执行分析
        user_seq_lengths, window_stats, active_users_stats = analyze_sequence_length_by_time_window(
            data_path,
